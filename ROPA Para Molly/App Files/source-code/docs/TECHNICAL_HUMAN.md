...
### 6. Escrow System (`src/server/routers/match.ts` + `offer.ts`)
ROPA acts as a payment escrow for all paid offers. The lifecycle is:
```
offer.accept → escrowStatus = 'held'
  → Both parties tap "Confirm Delivery" independently
  → match.confirmDelivery checks buyerConfirmedAt + sellerConfirmedAt
  → Both confirmed → match auto-completes, escrowStatus = 'released'
  → If dispute → escrowStatus = 'disputed', admin resolves via resolveDispute
```
The `ropaHeldAmount` field on `Offer` stores the overbid premium (difference between offer and listing price) which ROPA retains as a monetization mechanism. This makes the escrow the core revenue engine.

### 7. Offer Timer (`src/app/offers/page.tsx`)
All pending offers display a live countdown timer (per-second updates via `setInterval`). When under 1 hour remaining, the timer pulses red with a CSS animation to signal urgency. The `expiresAt` field on `Offer` is set to 24h from creation.
...
| OAuth | Email/password + mock login | Add `GoogleProvider` to `src/lib/auth.ts` |
| Push Notifications | Service worker pre-wired | Add `FIREBASE_SERVER_KEY` + trigger sends from match/offer events |
| Stripe | Escrow data model + UI complete | Wire Stripe Connect for real payment holds (escrowStatus transitions ready) |
| Profile Edit | ✅ Done | `/profile/edit` fully implemented |
| Review System | ✅ Done | `match.createReview` + `ReviewModal` wired |
