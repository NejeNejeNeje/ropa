...
```
Framework:     Next.js 16 (App Router, "use client" / "use server" model)
...
| Procedure | Type | Auth | Description |
|---|---|---|---|
| `getAll` | query | 🔒 | All matches for current user with last message + offer escrow data. |
| `accept` | mutation | 🔒 | Set match status to 'accepted'. |
| `complete` | mutation | 🔒 | Mark swap done (legacy). Awards 20 karma each, creates SwapBuddy. |
| `confirmDelivery` | mutation | 🔒 | **Escrow dual-confirm**: each party confirms independently. Both confirmed → auto-completes match, releases escrow, awards karma. |
| `openDispute` | mutation | 🔒 | Either party can open a dispute with a reason. Pauses escrow release, sets match/offer to 'disputed'. |
| `resolveDispute` | mutation | 🔒 | **Admin-only**: force-release or force-refund disputed escrow. |
...
| `createReview` | mutation | 🔒 | Leave a 1-5 star review after completed swap. |
| `getReviewStatus` | query | 🔒 | Check if current user already reviewed a match. |
...
| Procedure | Type | Auth | Description |
|---|---|---|---|
| `create` | mutation | 🔒 | Submit offer. Calculates sellerScore + distance. Auto-declines lowballs. Stores `ropaHeldAmount` for OVERBIDs. |
| `accept` | mutation | 🔒 | Accept offer → atomic tx: creates Match, declines others, awards karma. Sets `escrowStatus='held'`. |
| `decline` | mutation | 🔒 | Seller declines offer. Sets `escrowStatus='refunded'`. |
...
| `getForSeller` | query | 🔒 | All offers on seller's listings (auto-expires stale + sets `escrowStatus='refunded'`). |
...
7. **Escrow System:** ROPA acts as a payment escrow for all paid offers. The flow is: offer accepted → `escrowStatus='held'` → both parties independently tap "Confirm Delivery" → match auto-completes → `escrowStatus='released'`. If a dispute is opened, escrow pauses until admin resolves (force-release or force-refund). The `ropaHeldAmount` field stores the overbid premium (difference between offer and asking price) that ROPA retains.

8. **Dual-Confirmation Delivery:** Matches require BOTH `buyerConfirmedAt` AND `sellerConfirmedAt` to be set before the swap is completed and funds are released. This prevents one-sided confirmation fraud.

9. **Offer Expiry Timer:** All pending offers have a 24h countdown timer displayed live on the UI (per-second updates). When under 1 hour remaining, the timer pulses red to signal urgency.
