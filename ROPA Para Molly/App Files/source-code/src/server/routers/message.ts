...
export const messageRouter = router({
    send: protectedProcedure.input(z.object({
        ...
        // Verify caller is a member of this match
        const matchCheck = await ctx.prisma.match.findUnique({
            where: { id: input.matchId },
            select: { userAId: true, userBId: true },
        });
        if (!matchCheck || (matchCheck.userAId !== ctx.userId && matchCheck.userBId !== ctx.userId)) {
